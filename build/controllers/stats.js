...
export const getDashboardStats = TryCatch(async (req, res, next) => {
    let stats;
    if (myCache.has("dashboard_stats"))
        stats = JSON.parse(myCache.get("dashboard_stats"));
    else {
        ...
        const [currentMonthProducts, currentMonthUsers, currentMonthOrders, lastMonthProducts, lastMonthUsers, lastMonthOrders, productCount, userCount, orders, lastSixMonthsOrders, categories, maleUserCount, latestTransaction] = await Promise.all([
        ...
        const modifyLatestTransaction = latestTransaction.map(t => ({
            _id: String(t._id),
            amount: t.total,
            discount: t.discount,
            status: t.status,
...
export const getPieCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has("pie_stats")) {
        pieCharts = JSON.parse(myCache.get("pie_stats"));
    }
    else {
        const [processingOrder, shippedOrder, delieveredOrder, categories, productCount, outOfStock, allOrders, allUsers, customers, admins] = await Promise.all([
        ...
        const productCategoriesRatio = await getInventory({ categories, productCount });
...
export const getBarCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has("bar_stats")) {
        barCharts = JSON.parse(myCache.get("bar_stats"));
    }
    else {
        const today = new Date();
        ...
        const [sixMonthsAgoProducts, sixMonthsAgoUsers, tweleveMonthsAgoOrders] = await Promise.all([
...
export const getLineCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has("line_stats")) {
        lineCharts = JSON.parse(myCache.get("line_stats"));
    }
    else {
        const today = new Date();
        const twelveMonthsAgo = new Date();
        ...
        const [twelveMonthsAgoProducts, tweleveMonthsAgoUsers, tweleveMonthsAgoOrders] = await Promise.all([
